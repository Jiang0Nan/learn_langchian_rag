...
from lear_langchain.ragflow.api.db import LLMType
from lear_langchain.ragflow.api.db.services.llm_service import LLMBundle
from lear_langchain.ragflow.deepdoc.parser import DocxParser, ExcelParser, HtmlParser, JsonParser, MarkdownParser, PdfParser, TxtParser
from lear_langchain.ragflow.deepdoc.parser.figure_parser import VisionFigureParser, vision_figure_parser_figure_data_wrapper
from lear_langchain.ragflow.deepdoc.parser.pdf_parser import PlainParser, VisionParser
from lear_langchain.ragflow.rag.nlp import concat_img, find_codec, naive_merge, naive_merge_with_images, naive_merge_docx, rag_tokenizer, tokenize_chunks, tokenize_chunks_with_images, tokenize_table
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    doc = {
        "docnm_kwd": filename,
        "title_tks": rag_tokenizer.tokenize(re.sub(r"\.[a-zA-Z]+$", "", filename)) #切分
    }
    doc["title_sm_tks"] = rag_tokenizer.fine_grained_tokenize(doc["title_tks"])#进一步切分 例如'非奈利酮片 drug description 190125'    =》   '非奈 利酮 片 drug description 190125'
    res = []
    pdf_parser = None
    section_images = None
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        if layout_recognizer == "DeepDOC":
            ...
            try:
                # vision_model = LLMBundle(kwargs["tenant_id"], LLMType.IMAGE2TEXT)
                vision_model = LLMBundle("2ed4c2b4951211f0ac37269674914e0e", LLMType.IMAGE2TEXT)
                callback(0.15, "Visual model detected. Attempting to enhance figure extraction...")
            except Exception:
                vision_model = None
            ...
            if vision_model:
                ...
                #sections是文本加上页码范围，坐标位置信息，table包含了原始图片和位置信息，页码，描述信息
            res = tokenize_table(tables, doc, is_english)
            callback(0.8, "Finish parsing.")
    ...
    if section_images:
        ...
        res.extend(tokenize_chunks(chunks, doc, is_english, pdf_parser)) #对文本的图片进行裁剪、添加位置信息、以及对内容进行tokenizer
