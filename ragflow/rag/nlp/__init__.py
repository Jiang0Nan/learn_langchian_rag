...
from lear_langchain.ragflow.rag.utils import num_tokens_from_string
...
def tokenize_chunks(chunks, doc, eng, pdf_parser=None):
    ...
    for ii, ck in enumerate(chunks):
        ...
        if pdf_parser:
            try:
                d["image"], poss = pdf_parser.crop(ck, need_position=True) # 裁剪图片
                add_positions(d, poss)# 添加位置
                ck = pdf_parser.remove_tag(ck) # 移除位置等标签
...
def naive_merge(sections, chunk_token_num=128, delimiter="\n。；！？", overlapped_percent=0):
    # 把多个短文本段（sections）合并成合适大小的“文本块”（chunk），用于后续 embedding 或 RAG 检索。
    # 支持：
        # 控制最大 token 数
        # 设置切分符号（句号、顿号、感叹号等）
        # 设置 chunk 之间的 overlap 重叠比例
        # 保留 位置信息（如页码）
    from lear_langchain.ragflow.deepdoc.parser.pdf_parser import RAGFlowPdfParser
    if not sections:
        return []
    if isinstance(sections[0], type("")):
        sections = [(s, "") for s in sections]
    ...
    def add_chunk(t, pos):
        ...
        if cks[-1] == "" or tk_nums[-1] > chunk_token_num * (100 - overlapped_percent)/100.:
            if cks:
                overlapped = RAGFlowPdfParser.remove_tag(cks[-1])# 去除 chunk 中的特殊 tag例如@@9##
                t = overlapped[int(len(overlapped)*(100-overlapped_percent)/100.):] + t # 重复部分
            if t.find(pos) < 0:
                t += pos
            cks.append(t)
            tk_nums.append(tnum)
    ...
    def add_chunk(t, image, pos=""):
        ...
        if cks[-1] == "" or tk_nums[-1] > chunk_token_num:
            ...
            if result_images[-1] is None:
                ...
                result_images[-1] = concat_img(result_images[-1], image)#图像拼接
            tk_nums[-1] += tnum
...
def concat_img(img1, img2):
    # 将两个图像 img1 和 img2 垂直拼接，并返回新的合成图像
