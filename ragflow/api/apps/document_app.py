...
from lear_langchain.ragflow.api.constants import FILE_NAME_LEN_LIMIT, IMG_BASE64_PREFIX
from lear_langchain.ragflow.api.db import VALID_FILE_TYPES, VALID_TASK_STATUS, FileSource, FileType, ParserType, TaskStatus
from lear_langchain.ragflow.api.db.db_models import File, Task
...
from lear_langchain.ragflow.api.db.services.task_service import TaskService, cancel_all_task_of, queue_tasks
from lear_langchain.ragflow.api.db.services.user_service import UserTenantService
from lear_langchain.ragflow.api.utils import get_uuid
from lear_langchain.ragflow.api.utils.api_utils import (
    get_data_error_result,
    get_json_result,
    server_error_response,
    validate_request,
)
from lear_langchain.ragflow.api.utils.file_utils import filename_type, get_project_base_directory, thumbnail
from lear_langchain.ragflow.api.utils.web_utils import CONTENT_TYPE_MAP, html2pdf, is_valid_url
...
def run():
    ...
    try:
        ...
        for id in req["doc_ids"]:
            ...
            if not e:
                return get_data_error_result(message="Document not found!")
            # 判断任务运行状态,如果任务处于取消,判断之前是否处于运行状态,是的话则取消任务,如果没有运行则不用取消
            if str(req["run"]) == TaskStatus.CANCEL.value:
                if str(doc.run) == TaskStatus.RUNNING.value:
                    cancel_all_task_of(id)
                else:
                    return get_data_error_result(message="Cannot cancel a task that is not in RUNNING status")
            # 重新运行且之前已经运行完成则清空chunk
