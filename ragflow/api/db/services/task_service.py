...
from lear_langchain.ragflow.deepdoc.parser import PdfParser
from peewee import JOIN
from lear_langchain.ragflow.api.db.db_models import DB, File2Document, File
from lear_langchain.ragflow.api.db import StatusEnum, FileType, TaskStatus
from lear_langchain.ragflow.api.db.db_models import Task, Document, Knowledgebase, Tenant
...
from lear_langchain.ragflow.api.utils import current_timestamp, get_uuid
from lear_langchain.ragflow.deepdoc.parser.excel_parser import RAGFlowExcelParser
from lear_langchain.ragflow.rag.settings import get_svr_queue_name
...
def queue_tasks(doc: dict, bucket: str, name: str, priority: int):
    ...
    parse_task_array = []
    # pdf一页拆成一个任务，excel3000行一个任务，其他只有一个任务
    ...
    # 获取当前文档的分块配置，包括划分策略、parser 配置等
    chunking_config = DocumentService.get_chunking_config(doc["id"])
    for task in parse_task_array:
        hasher = xxhash.xxh64()
        for field in sorted(chunking_config.keys()):
            # 删除对生成摘要无用的配置,如果不删除它们，小小一个 UI 配置变化，也会让摘要变不同 → 导致 明明内容相同却无法复用。
            if field == "parser_config":
                for k in ["raptor", "graphrag"]:
                    if k in chunking_config[field]:
                        del chunking_config[field][k]
            # 对配置和 from_page/to_page 进行哈希
            hasher.update(str(chunking_config[field]).encode("utf-8"))
        for field in ["doc_id", "from_page", "to_page"]:
            hasher.update(str(task.get(field, "")).encode("utf-8"))
        # digest 是用来标识一个「切分任务」是否 和之前的一模一样 的：
        # 如果 digest 一样，说明之前已经执行过，可以复用；
        # 如果 digest 不一样，说明文档、页数或配置不同，必须重新执行。
...
def reuse_prev_task_chunks(task: dict, prev_tasks: list[dict], chunking_config: dict):
    ...
    这个函数检查以前完成的任务中的块是否可以重新用于
    当前任务，这可以显著提高处理效率。它匹配
    基于页面范围和配置摘要的任务。
    Args:
    ...
    if prev_task["progress"] < 1.0 or not prev_task["chunk_ids"]:
        return 0 #找到了但是没有完成或者没有chunk_ids
    # 复用逻辑
    ...
    return len(task["chunk_ids"].split())#复用的个数
