...
from lear_langchain.ragflow.api.utils.file_utils import get_project_base_directory
from lear_langchain.ragflow.deepdoc.vision import OCR, LayoutRecognizer, Recognizer, TableStructureRecognizer
from lear_langchain.ragflow.rag.app.picture import vision_llm_chunk as picture_vision_llm_chunk
from lear_langchain.ragflow.rag.nlp import rag_tokenizer
from lear_langchain.ragflow.rag.prompts import vision_llm_describe_prompt
from lear_langchain.ragflow.rag.settings import PARALLEL_DEVICES
...
class RAGFlowPdfParser:
    ...
    def _table_transformer_job(self, ZM):
        ...
        tbcnt = np.cumsum(tbcnt) #逐项累加 得到每页的结束索引 tbcnt = np.array([2, 3, 5]) =》array([ 2,  5, 10])
        # 把每页识别出来的表格组件坐标映射到整个 PDF 页面坐标系
        for i in range(len(tbcnt) - 1):  # for page
            pg = []
            for j, tb_items in enumerate(
                    recos[tbcnt[i]: tbcnt[i + 1]]):  # for table 迪i也的所有表格
                poss = pos[tbcnt[i]: tbcnt[i + 1]]
                for it in tb_items:  # for table components
                    it["x0"] = (it["x0"] + poss[j][0])#poss[j] 是 表格在原图的左上角坐标偏移，用于把表格组件的局部坐标映射到整页图像坐标
                    it["x1"] = (it["x1"] + poss[j][0])
                    it["top"] = (it["top"] + poss[j][1])
                    it["bottom"] = (it["bottom"] + poss[j][1])
                    for n in ["x0", "x1", "top", "bottom"]:
                        it[n] /= ZM #坐标缩放回 原 PDF 单位。
                    it["top"] += self.page_cum_height[i]
                    it["bottom"] += self.page_cum_height[i]
                    it["pn"] = i #标记页码
                    it["layoutno"] = j #标记表格编号
                    pg.append(it)
    ...
    def _text_merge(self):
        ...
        while i < len(bxs) - 1:
            ...
            if abs(self._y_dis(b, b_)
                ...
                # merge 如果两框的垂直距离小于该页字符高度的1/3就任务是同一行,进行合并
                bxs[i]["x1"] = b_["x1"]
            ...
            #考虑中文标点或特殊情况调整横向距离阈值可以让断行在句号、逗号、括号处更自然
            dis_thr = 1
            dis = b["x1"] - b_["x0"]
    ...
    def _extract_table_figure(self, need_image, ZM, return_html, need_position, separate_tables_figures=False):
        ...
        while i < len(self.boxes):
            ...
            #检查是否是图，表（is_caption正则匹配），表格标题，标题，图像标题，参考信息、说明文字（layout_typ标出的），是的话不可合并
            ...
            if self.boxes[i]["layout_type"] == "table":
                if re.match(r"(数据|资料|图表)*来源[:： ]", self.boxes[i]["text"]): #删除无用文本，比如数据的来源等
                    self.boxes.pop(i)
                    continue
        ...
        i = len(tbls) - 1
        while i - 1 >= 0:
            k0, bxs0 = tbls[i - 1]  # 倒数第二个tabel
            k, bxs = tbls[i]#最后一个tabel
            i -= 1
            if k0 in nomerge_lout_no:#已经添加过 或者说不是表格是描述信息
                continue
            if bxs[0]["page_number"] == bxs0[0]["page_number"]:#不在同一页
                continue
            if bxs[0]["page_number"] - bxs0[0]["page_number"] > 1:#相隔多页
                continue
            mh = self.mean_height[bxs[0]["page_number"] - 1]
            if self._y_dis(bxs0[-1], bxs[0]) > mh * 23:#距离太远
                continue
            tables[k0].extend(tables[k])#感觉实际上是合并第一次出现的描述信息或者表格
            del tables[k]

        def x_overlapped(a, b):
            return not any([a["x1"] < b["x0"], a["x0"] > b["x1"]])

        # find captions and pop out 标题/说明绑定到图表
        i = 0
        while i < len(self.boxes):#这是删除图表后的
            c = self.boxes[i]
            # mh = self.mean_height[c["page_number"]-1]
            if not TableStructureRecognizer.is_caption(c): #判断你是不是标题或者描述信息
                i += 1
                continue
            ...
            #    continue
            if tv < fv and tk:
                tables[tk].insert(0, c)# 绑定描述信息与表格
                logging.debug(
                    "TABLE:" +
                    self.boxes[i]["text"] +
                    "; Cap: " +
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        try:
            with sys.modules[LOCK_KEY_pdfplumber]:
                with (pdfplumber.open(fnm) if isinstance(fnm, str) else pdfplumber.open(BytesIO(fnm))) as pdf:
                    self.pdf = pdf
                    # 取 PDF 中从 page_from 到 page_to 的页对象列表（左闭右开）。
                    # self.pdf 是一个 pdfplumber.PDF 对象。
                    # 给每一页加上索引 i（在这里没实际用到，只是循环惯用写法）。
                    # pdfplumber 页对象的 to_image() 方法：把 PDF 页渲染成图像（PIL Image）。
                    # 参数：
                    # resolution=72 * zoomin：图像分辨率，默认 PDF 是 72 dpi，zoomin 可以放大。
                    # antialias=True：启用抗锯齿，使文字和线条更平滑。
                    # to_image() 返回一个 PageImage 对象，.annotated 是渲染后的图像（PIL Image），包含 PDF 上的标注（annotations）信息。
                    # 所以 self.page_images 得到的是 一组 PIL 图像对象。

                    # 从pdf的[page_from,page_to)提取为每页是一个72*zoomin分辨率的抗锯齿的pil图片
                    self.page_images = [p.to_image(resolution=72 * zoomin, antialias=True).annotated for i, p in
                                        enumerate(self.pdf.pages[page_from:page_to])]

                    try:
                        self.page_chars = [[c for c in page.dedupe_chars().chars if self._has_color(c)] for page in self.pdf.pages[page_from:page_to]]
                        # for i ,images in enumerate(self.page_images):
                        #     has_color_char = self.page_chars[i]
                        #     draw_image = ImageDraw.Draw(images)
                        #     page_height=images.height
                        #     for item in has_color_char:
                        #         x0=item.get('x0','')
                        #         top=item.get('top','')
                        #         x1=item.get('x1','')
                        ...
                        #         y0, y1 = sorted([y0, y1])
                        #
                        #         draw_image.rectangle([x0, y0, x1, y1],outline=item.get('outline','red'))
                        #     plt.imshow(images)
                        #     plt.axis("off")
                        #     plt.title("提取图片")
                        #     plt.show()
        ...
        # 检测是否存在乱码，从页面中最多随机抽取100个字符，拼接成字符串，如果连续出现30个符号/英文字母，数字那么就是乱码的
        ...
        async def __img_ocr(i, id, img, chars, limiter):
            j = 0
            #这个判断用于 检测 OCR 时漏掉的空格 —— 如果两个英文字符之间的间隔大于一定阈值（宽度的一半），就认为应该插入一个空格。
            while j + 1 < len(chars):
            ...
            if callback and i % 6 == 5:
                callback(progress=(i + 1) * 0.6 / len(self.page_images), msg="")
    ...
    def crop(self, text, ZM=3, need_position=False):
        ...
        poss.insert(0, ([pos[0][0]], pos[1], pos[2], max(
            0, pos[3] - 120), max(pos[3] - GAP, 0))) # 裁一个“原图区域上面 120 像素高”的区域作为上边缘补充背景区域
        pos = poss[-1]# 最后一个原始区域
        poss.append(([pos[0][-1]], pos[1], pos[2], min(self.page_images[pos[0][-1]].size[1] / ZM, pos[4] + GAP),
                     min(self.page_images[pos[0][-1]].size[1] / ZM, pos[4] + 120))) # 裁一个“原图区域下方 120 像素”的区域，作为下边缘的补充背景区域。
...
class VisionParser(RAGFlowPdfParser):
    ...
    def __call__(self, filename, from_page=0, to_page=100000, **kwargs):
        callback = kwargs.get("callback", lambda progress, msg: None)
