...
model_ollama = ChatOllama(
    ...
    num_predict = -1,#生成文本时要预测的最大标记数。(默认:` ` 128 `, ` `- 1 ` =无限代，` `- 2 ` =填充上下文)""
...
messages = [SystemMessage(content="你是一个是诗人"),
            HumanMessage(content="以月亮为题作一首七言诗"),]

messages2 = [SystemMessage(content="你是一位乐于助人的智能小助手"),
 HumanMessage(content="请帮我介绍一下什么是AIGC"),]

# ====================1. invoke
res = model_init.invoke(messages)
print(res.content)
res = model_ollama.invoke(messages)
print(res.content)

# ====================2.stream
for chunk in model_init.stream(messages):
    print(chunk.content ,end="", flush=True)

for chunk in model_ollama.stream(messages):
    print(chunk.content, end="", flush=True)
...
async def run_async_methods():
    ...
    # ====================3. ainvoke
    res = await model_init.ainvoke(messages)
    print(res)
    res = await model_ollama.ainvoke(messages)
    print(res)

    # ====================4. abacth
    res =  await  model_init.abatch([messages,messages2])
    print(f"abatch===model_init:{res}")
    print(res)
    contents = [msg.content for msg in res]
    for content in contents:
        print(content, "\n---\n")

    res = await model_ollama.abatch([messages,messages2])
    print(f"abatch===model_ollama:{res}")
    contents = [msg.content for msg in res]
    for content in contents:
        print(content, "\n---\n")
