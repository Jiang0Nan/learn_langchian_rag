...
from parctise.utils import get_project_base_directory
...
class RagTokenizer:
    ...
    def loadDict_(self, fnm):
        ...
        try:
            ...
            path = os.path.join(get_project_base_directory())
            logging.exception(f"[HUQIE]:Build trie {path},{fnm} failed")

    def __init__(self, debug=False):
        self.DEBUG = debug
        self.DENOMINATOR = 1000000
        self.DIR_ = os.path.join(get_project_base_directory(), "parctise/res", "huqie")
    ...
    def tokenize(self, line):
        ...
        # 根据语言类型拆分
        # 英文：分词 + 词形还原 + 词干提取
        # 中文：正向最大匹配、反向最大匹配对齐，二者不一致时用 DFS +排序优化切分
    ...
    def fine_grained_tokenize(self, tks):
        # 根据中英文比例，决定是否需要深入处理
        # 对中文或长英文 token，进一步尝试 DFS 拆分
        # 用排序器 sortTks_ 找到“最优分词”
        # 对一些特殊情况（过短词、数字、英文拼写）进行保留或跳过拆分
        # 最后进行一次 english_normalize_（可能是大小写统一、过滤 stopword、统一符号等）
        # 例如非奈利酮片 drug description 190125'    =》'非奈 利酮 片 drug description 190125'
        tks = tks.split()
        ...
        res = []
        for tk in tks:
            if len(tk) < 3 or re.match(r"[0-9,\.-]+$", tk): # 如果是数字则不动
                res.append(tk)
                continue
            tkslist = []
            if len(tk) > 10:
